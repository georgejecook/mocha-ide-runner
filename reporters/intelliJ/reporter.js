...
function extractErrInfo(err) {
  ...
  if (index >= 0) {
    stack = treeUtil.fixFileLocations(stack);
...
function addStdErr(testNode, err) {
  if (err != null) {
    if (util.isString(err)) {
      ...
      if (errInfo != null) {
        var stack = errInfo.stack;
        if (stack){
          stack = treeUtil.fixFileLocations(stack);
        }
        var out = errInfo.message || stack;
        if (errInfo.message && stack) {
          out = errInfo.message + '\n' + stack;
        }
...
function finishTestNode(tree, test, err) {
  ...
  if (err) {
    ...
    var stack = treeUtil.fixFileLocations(errInfo.stack);
    testNode.setOutcome(Tree.TestOutcome.FAILED, test.duration, errInfo.message, stack, actualStr, expectedStr);
  }
  else {
